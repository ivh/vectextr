...
def test_extract_basic(fname, slope=None, slitdeltas=None):
    ...
    if slitdeltas is None and slope is None:
        ...
        slitdeltas_os = np.linspace(
            -slope * nrows / 2, slope * nrows / 2, ny, dtype=np.float64
        )
    else:  # oversample incoming slitdeltas
        slitdeltas_os = np.interp(
            np.linspace(0, nrows - 1, ny), np.arange(nrows), slitdeltas
        )
    ...
    plt.imshow(
        residuals, origin="lower", aspect="auto", cmap="RdBu_r", vmin=-vmax, vmax=vmax
    )
...
def run_single_test(fname, slope=None, slitdeltas=None):
    ...
    try:
        ...
        result, sL, sP, model, unc = test_extract_basic(
            fname=fname, slope=slope, slitdeltas=slitdeltas
        )
        if result == -1:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--single-test":
        ...
        if fname.endswith("discontinuous.fits"):
            slitdeltas = np.concatenate(
                [
                    ...
                    np.linspace(-5, 5, 100)[30:60]
                    - 4.0,  # Rows 30-59: Extra 4 pixel left shift
                    np.linspace(-5, 5, 100)[60:]
                    - 8.0,  # Rows 60-99: Extra 8 pixel left shift
                ]
            )
        ...
        # Handle any data file with computed slitdeltas from make_slitdeltas.py
            ...
            f'slitdeltas_{os.path.basename(fname).replace(".fits", "")}.npz'
        ):
